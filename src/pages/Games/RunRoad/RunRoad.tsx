...
const RunRoad: React.FC = () => {
  ...
  const [highScore, setHighScore] = useState(() => {
    const saved = localStorage.getItem('runroad-highscore');
  ...
  const initGame = useCallback(() => {
    ...
    // Textures
    const createTexture = (width: number, height: number, rects: { x: number; y: number; w: number; h: number }[]) => {
      ...
      constructor(index: number) {
        ...
        switch (this.type) {
          ...
          case 'car':
            ...
            const carOccupied = new Set<number>();
            this.vechicles = [1, 2, 3].map(() => {
              const vechicle = createCar();
              let position: number;
              do {
                position = Math.floor(Math.random() * (columns / 2));
              } while (carOccupied.has(position));
              carOccupied.add(position);
          ...
          case 'truck':
            this.mesh = createRoad();
            this.direction = Math.random() >= 0.5;
            const truckOccupied = new Set<number>();
            this.vechicles = [1, 2].map(() => {
              const vechicle = createTruck();
              let position: number;
              do {
                position = Math.floor(Math.random() * (columns / 3));
              } while (truckOccupied.has(position));
              truckOccupied.add(position);
    ...
    const move = (direction: string) => {
      ...
      const finalPositions = moves.reduce(
        (position, m) => {
          if (m === 'forward') return { lane: position.lane + 1, column: position.column };
          if (m === 'backward') return { lane: position.lane - 1, column: position.column };
          if (m === 'left') return { lane: position.lane, column: position.column - 1 };
          if (m === 'right') return { lane: position.lane, column: position.column + 1 };
          return position;
        },
    ...
    const animate = (timestamp: number) => {
      ...
      if (lanes[currentLane]?.type === 'car' || lanes[currentLane]?.type === 'truck') {
        ...
        const vechicleLength = lanes[currentLane].type === 'car' ? 60 : 105;
        lanes[currentLane].vechicles?.forEach((vechicle) => {
          ...
          if (chickenMaxX > carMinX && chickenMinX < carMaxX) {
            ...
            if (currentLane > highScore) {
              setHighScore(currentLane);
              localStorage.setItem('runroad-highscore', currentLane.toString());
            }
            setScreen('gameover');
          }
        });
    ...
    // Store game reference for controls and cleanup
    gameRef.current = {
      move,
      cleanup: () => {
    ...
    <div className="runroad-game">
        ...
        <div className="game-hud">
          ...
          <div className="controls">
            ...
            <div className="control-row">
              <button className="control-btn left" onClick={() => handleMove('left')}>←</button>
              <button className="control-btn down" onClick={() => handleMove('backward')}>↓</button>
              <button className="control-btn right" onClick={() => handleMove('right')}>→</button>
            </div>
          </div>
        </div>
      )}
      ...
      {screen === 'gameover' && (
        <div className="game-menu">
          ...
          <p className="highscore">🏆 High Score: {highScore}</p>
          <button onClick={initGame} className="menu-btn play-btn">🔄 Try Again</button>
